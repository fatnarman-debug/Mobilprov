  ...
  try {
    const topic = await prisma.topic.create({
      data: {
        ...
        description: 'Kapitel 11 - Sveriges samarbete med omvärlden och säkerhetspolitik (Sverige i fokus).',
        category: 'Samhälle',
        order: 11,
        isPublished: true,
      },
    ...
    await prisma.material.create({
      data: {
        topicId: topic.id,
        type: 'PDF',
        title: 'Sverige i fokus - Kapitel 11',
    ...
    return NextResponse.json({ 
      ...
      message: 'Sverige och omvärlden added successfully!' 
    });
  } catch (error: any) {
