  ...
  try {
    const topic = await prisma.topic.create({
      data: {
        ...
        description: 'Kapitel 13 - Svenska traditioner, helgdagar och kulturella högtider (Sverige i fokus).',
        category: 'Kultur',
        order: 13,
        isPublished: true,
      },
    ...
    await prisma.material.create({
      data: {
        topicId: topic.id,
        type: 'PDF',
        title: 'Sverige i fokus - Kapitel 13',
    ...
    return NextResponse.json({ 
      ...
      message: 'Traditioner och högtider added successfully!' 
    });
  } catch (error: any) {
